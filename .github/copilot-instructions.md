...
- **Subject-grouped patches (JP6.x `nvidia-oot/`)**: JetPack 6.x out-of-tree patches are organized by *subject/feature area*, not strictly one-file-per-patch. The canonical set (real files in `6.0/`, symlinked from the other 6.x dirs — see below) is: `0001-embedded-metadata-support-for-D4xx` (Tegra VI/camera-core changes **plus** the `d4xx.o` build wiring in `drivers/media/i2c/Makefile`), `0002-max9295-max9296-SerDes-support-for-D4xx` (the GMSL serializer **and** deserializer pair together — they are the two halves of one link, so they share a patch), `0003-Adding-max96712-support-for-D4xx` (the MAX96712 deserializer, `max96712.c` only), `0004-Fix-y12i-calibration-stream`, and `0005-Runtime-tsc-rate-config`. Keep distinct subjects in distinct patches; do not fold a new subject into an existing one. (For the older `kernel/nvidia/` JP4/5 trees the historical one-patch-per-module split still applies.)
- **Per-version symlink dedup (`nvidia-oot/`)**: `6.0/` holds the real patch files; `6.1/` and `6.2/` symlink each patch that is byte-identical back to `6.0/` (or to `6.1/` for the tsc patch), and only carry a real file where that version's content genuinely differs (e.g. `6.2/0001` is a real variant; `6.1/0005` and `6.2/0005` are the variant-B tsc). `6.2.1/` and `6.2.2/` are whole-directory symlinks to `6.2/`. When editing a shared patch, edit the real `6.0/` file; never duplicate content that should be a symlink.
