...
- `DS5_FW_CSI_PT` (`0x2E`) is the D40x FW CSI-PT mode selector for the OV9782 sensor — not a MIPI wire DT. Written to `DS5_RGB_STREAM_DT` to activate FW CSI passthrough mode. The sole active use is the D401 RAW8 CSI passthrough format (`mbus_code = MEDIA_BUS_FMT_SRGGB8_1X8`):
  ...
  - Set SerDes PIPE_X_DT = `0x2A` — aligns MAX9296 routing with the actual RAW8 wire DT. In `ds5_configure()`, guard with `mbus_code == MEDIA_BUS_FMT_SRGGB8_1X8` after the standard `data_type1` init, setting `data_type1 = MIPI_CSI2_TYPE_RAW8`.
